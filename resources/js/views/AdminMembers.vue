    ...
    <div class="admin-tests">
            ...
            <div class="d-block mb-4 mb-md-0">
                <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                    <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Члены комиссии</li>
                    </ol>
                </nav>
                <h2 class="h4">Члены комиссии</h2>
                <p class="mb-0">Список членов</p>
            </div>
            <div class="d-flex justify-content-end">
                <router-link :to="{path: '/admin/member'}" class="btn btn-sm btn-secondary d-inline-flex align-items-center">
        ...
        <div class="card card-body border-0 shadow table-wrapper table-responsive">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr v-for="member in members" :key="member.id">
                            ...
                            <td><span class="fw-normal">{{ member.position }}</span></td>
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu py-0">
                                        ...
                                        <router-link class="dropdown-item" :to="{path: '/admin/member/' + member.id}"><span class="fas fa-edit me-2"></span>Редактировать</router-link>
                                        <a class="dropdown-item text-danger rounded-bottom" href="javascript:;" @click="getMemberId(member.id)" data-bs-toggle="modal" data-bs-target="#modal-remove"><span class="fas fa-trash-alt me-2"></span>Удалить</a>
                                    </div>
                                </div>
                            </td>
                        </tr>                                
                ...
                <div class="card-footer px-3 border-0 d-flex flex-column flex-lg-row align-items-center justify-content-between">
                    <Pagination :total="total" :item="count_tests" @page-changed="loadListMembers"/>
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" @click.prevent="deleteMember(member.id)" data-bs-dismiss="modal">Удалить</button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.loadMembers();
            this.loadListMembers(this.page)
        },
        methods: {
            loadMembers() {
                ...
                .then(res => {
                    ...
                    if(page){
                        this.loadListMembers(page)
                    }else if(this.total > this.count_tests){
                        this.loadListMembers(1)
                    }
                    //setTimeout(() => { }, 500)
                })
            },
            gotoMember(id) {
                this.$router.push('/admin/member/show/'+id);
            },
            getMemberId(id) {
                if(id) this.member.id = id;
            },
            deleteMember(id){
                axios.post('/api/member/del/'+id)
                .then(res => {
                    if(res.data.status)
                        this.loadMembers();
                    else
            ...
            loadListMembers(pageNumber){
                var path = '/admin/members/page/'+pageNumber;
                var start_page = (this.count_tests * pageNumber) - this.count_tests;
                var end_page = pageNumber * this.count_tests;
                var arr_tests = [];
                var n = 0;
                for(var i=0; i<this.members_data.length; i++){
                    if(i >= start_page && i < end_page){
                        n++;
                        arr_tests.push(this.members_data[i]);
                    }
                }
                this.members = arr_tests;
                this.curr_count = n;
                if (this.$route.path !== path){
                    this.$router.push('/admin/members/page/'+pageNumber);
                }
            }
        }
    }
