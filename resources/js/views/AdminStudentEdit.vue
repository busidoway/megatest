        ...
        <div class="py-4">
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                    ...
                    <li class="breadcrumb-item"><router-link :to="{path: '/admin/students'}">Сотрудники</router-link></li>
                    <li class="breadcrumb-item active" aria-current="page">Добавление сотрудника</li>
                </ol>
            </nav>
            <div class="d-flex justify-content-between w-100 flex-wrap mb-3">
                <div class="mb-3 mb-lg-0">
                    <h1 class="h4">Добавление сотрудника</h1>
                </div>
            </div>
            <div>
                    ...
                    <div class="card-body">
                        ...
                        <form>
                            ...
                            <div class="row mx-0 mb-4">
                                <input type="text" name="name" id="nameStudent" v-model="form.name" :class="[{ 'is-invalid':state.form.error, 'form-control-disabled':state.form.disabled }]" class="form-control" placeholder="Ф.И.О." @input="changeName(form.name)" required>
                            </div>
                            <div class="row mx-0 mb-3">
                                <input type="text" name="position" id="positionStudent" v-model="form.position" :class="[{ 'is-invalid':state.form.error, 'form-control-disabled':state.form.disabled }]" class="form-control" placeholder="Должность" @input="changePosition(form.position)" required>
                            </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.editStudent(this.$route.params.id);
        },
        methods: {
            store() {
                ...
                if(id) {
                    api_rout = '/api/student/update/' + id;
                }else{
                    api_rout = '/api/student/store';
                }
                ...
                .then(res => {
                    if (res.data.status) {
                        if(res.data.store) this.$router.push('/admin/student/'+ res.data.student.id);
